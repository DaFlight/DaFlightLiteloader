...
public class EntryBox7 extends GuiTextField implements IEntryBox
{
    ...
    public EntryBox7(int x, int y, int width, int height, String label, String defaultValue, boolean colour)
    {
        super(DaFlight.getMC().getMinecraft().fontRendererObj, x, y, width, height);
    ...
    public EntryBox7 setString(String s)
    {
        if (coloured)
        {
    ...
    {
        super.drawTextBox();
        hovered = mouseX >= this.xPos && mouseX <= this.xPos + this.getWidth() && mouseY >= this.yPos && mouseY <= this.yPos + this.height;
    }
